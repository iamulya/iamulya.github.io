...
> **Forcing Tool Use for Debugging**
> {:.title}
...
```mermaid
---
title: The SDK's process for converting MCP tools into usable FunctionTools.
---
graph TD
    A(Runner executes Agent) --> B{Agent has mcp_servers};
    B --> C[MCPUtil.get_all_function_tools];
    C -->|For each server| D(server.list_tools);
    D -->|Returns| E[List of MCPTool objects];
    E --> F[MCPUtil.to_function_tool];
    F -->|Creates| G(FunctionTool instance);
    G --> H{Agent's effective tool list};
    H --> I(LLM);
```
...
> **Handling JSON Schema Differences**
> {:.title}
