...
```mermaid
---
title: Execution flow of a `SequentialAgent`.
---
sequenceDiagram
    participant UserInput
    participant SequentialAgent as "user_onboarding_pipeline"
    participant SubAgent1 as "data_collection_agent"
    participant SubAgent2 as "email_validation_agent"
    participant SubAgent3 as "welcome_email_agent"
    participant SessionState

    UserInput->>SequentialAgent: run_async(initial_message)
    SequentialAgent->>SubAgent1: run_async(context)
    Note over SubAgent1,SessionState: SubAgent1 uses tool, updates SessionState (name, email)
    SubAgent1-->>SequentialAgent: Completes its run (Events yielded)
    SequentialAgent->>SubAgent2: run_async(context_with_updated_state)
    Note over SubAgent2,SessionState: SubAgent2 reads email, validates, updates SessionState (email_validated)
    SubAgent2-->>SequentialAgent: Completes its run (Events yielded)
    SequentialAgent->>SubAgent3: run_async(context_with_further_updated_state)
    Note over SubAgent3,SessionState: SubAgent3 reads state, sends email (conceptually)
    SubAgent3-->>SequentialAgent: Completes its run (Events yielded)
    SequentialAgent-->>UserInput: Final stream of all sub-agent events
...
```mermaid
---
title: Execution flow of a `ParallelAgent` managed by an orchestrator.
---
sequenceDiagram
    participant UserInput
    participant Orchestrator as "analysis_orchestrator"
    participant ParallelAgent as "text_analysis_parallel_tasks"
    participant SubAgentA as "sentiment_analyzer"
    participant SubAgentB as "keyword_extractor"
    participant LLM_A as "LLM for SubAgentA"
    participant LLM_B as "LLM for SubAgentB"
    participant FinalOutputProcessor as "Orchestrator's Logic / LLM"

    UserInput->>Orchestrator: run_async(analyze_text_message)
    Orchestrator->>ParallelAgent: run_async(context_with_text)

    par
        ParallelAgent->>SubAgentA: run_async(context)
        SubAgentA->>LLM_A: Prompt for sentiment
        LLM_A-->>SubAgentA: Sentiment result
        SubAgentA-->>ParallelAgent: Event (author=SubAgentA, branch=..., content=sentiment)
    and
        ParallelAgent->>SubAgentB: run_async(context)
        SubAgentB->>LLM_B: Prompt for keywords
        LLM_B-->>SubAgentB: Keyword results
        SubAgentB-->>ParallelAgent: Event (author=SubAgentB, branch=..., content=keywords)
    end

    ParallelAgent-->>Orchestrator: Yields interleaved/collected Events from SubAgentA & SubAgentB
    Orchestrator->>FinalOutputProcessor: Processes Events (e.g., summarizes or sends to its own LLM)
    FinalOutputProcessor-->>Orchestrator: Combined/Final Result
    Orchestrator-->>UserInput: Final summarized analysis
...
4. `check_draft_quality` logic runs:
    - If exit condition met: it calls `exit_loop(tool_context)`. This sets 
...
```mermaid
---
title: Execution flow of a `LoopAgent` with a sub-agent that can signal loop termination.
---
sequenceDiagram
    participant UserInput
    participant LoopAgent as "iterative_refinement_loop"
    participant SubAgent as "drafting_agent"
    participant LLM_Sub as "LLM for SubAgent"
    participant QualityTool as "check_draft_quality_tool"

    UserInput->>LoopAgent: run_async(initial_draft_info)
    loop Max Iterations or Until Escalate
        LoopAgent->>SubAgent: run_async(context)
        SubAgent->>LLM_Sub: Prompt (e.g., "Refine draft: ... based on feedback: ...")
        LLM_Sub-->>SubAgent: Request tool_call(check_draft_quality, args={draft: "new draft..."})
        SubAgent->>QualityTool: run_async(args, tool_context)
        alt Draft OK or Max Iterations
            QualityTool->>ToolContext: tool_context.actions.escalate = True
            QualityTool-->>SubAgent: Result (e.g., {"action":"exit"})
        else Draft Needs Work
            QualityTool-->>SubAgent: Result (e.g., {"action":"refine", "feedback":"more detail"})
        end
        SubAgent-->>LoopAgent: Event (from tool_response, may have escalate=True)
        LoopAgent-->> UserInput: Yields SubAgent's events
        alt Event has escalate=True
            LoopAgent->>LoopAgent: Terminate Loop
        end
    end
